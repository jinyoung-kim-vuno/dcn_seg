...
def generate_fc_densenet_dilated(gen_conf, train_conf):
    ...
    model = __generate_fc_densenet_ms(dimension, num_classes, multi_output, output_name, attention_loss,
...
def __generate_fc_densenet_ms(dimension, num_classes, multi_output, output_name, attention_loss, overlap_penalty_loss,
...
def organise_output(dimension, kernel_init, concat_axis, input, c_score_up, output_shape, activation, output_name,
    ...
    if attention_loss == 1:
        ...
        if dimension == 2:
            c_concate_fc = Conv3D(2, (1, 1), padding='same', kernel_initializer=kernel_init)(c_concate)
        elif dimension == 3:
...
def transition_up_dense_conv(dimension, multi_output, l, weight_decay, concat_axis, early_transition, nb_dense_block,
    ...
    for block_idx in range(nb_dense_block):
        ...
        if attn_gates:
            ...
            if dimension == 2:
                skip_connection = Conv3D(n_filters_keep, (1, 1), kernel_initializer=kernel_init, padding='same',
